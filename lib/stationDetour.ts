...
import { fuelPriceKey, type FuelStationPrice } from '@/lib/fuelPrices';
...
export function rankStationsForDetour(opts: {
  ...
  const withPrice = opts.stations.filter((s) => s.prices[key] != null && (s.distanceKm ?? 99) < 35);
  if (!withPrice.length) return [];

  const prices = withPrice.map((s) => s.prices[key]!);
  const refPrice =
    median(prices) ||
    opts.vehicle.defaultFuelPrice ||
    (opts.vehicle.fuelType === 'diesel' ? 1.65 : 1.8);

  const ranked: RankedStation[] = withPrice.map((s) => {
    const pricePerL = s.prices[key]!;
    ...
    const label = `${s.name}${city} · ${pricePerL.toFixed(3)} €/L · ${detourKm.toFixed(1)} km · ~${fuelToReachL.toFixed(1)} L · ${
